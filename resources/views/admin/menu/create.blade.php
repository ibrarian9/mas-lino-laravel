...
<div class="bg-white rounded-xl shadow-sm max-w-[600px]">
    <div class="px-5 py-3 border-b border-cream font-semibold text-sm text-primary">Tambah Menu Baru</div>
    <div class="p-5">
        <form action="{{ route('admin.menu.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="mb-4">
                <label for="nama_menu" class="block font-semibold text-sm mb-1.5 text-primary">Nama Menu</label>
                <input type="text" name="nama_menu" id="nama_menu" class="w-full px-4 py-3 border-2 border-border rounded-xl font-sans text-base focus:outline-none focus:border-secondary" value="{{ old('nama_menu') }}" required maxlength="150">
            ...
            <div class="mb-4">
                ...
                <select name="kategori" id="kategori" class="w-full px-4 py-3 border-2 border-border rounded-xl font-sans text-base focus:outline-none focus:border-secondary" required>
                    ...
                    <option value="bundling" {{ old('kategori') === 'bundling' ? 'selected' : '' }}>Bundling</option>
                </select>
            </div>
            <div class="mb-4">
                <label for="deskripsi" class="block font-semibold text-sm mb-1.5 text-primary">Deskripsi</label>
                <textarea name="deskripsi" id="deskripsi" class="w-full px-4 py-3 border-2 border-border rounded-xl font-sans text-base focus:outline-none focus:border-secondary resize-none" rows="3">{{ old('deskripsi') }}</textarea>
            </div>
            <div class="mb-4">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div class="mb-4">
                    ...
                    <input type="number" name="harga_normal" id="harga_normal" class="w-full px-4 py-3 border-2 border-border rounded-xl font-sans text-base focus:outline-none focus:border-secondary" value="{{ old('harga_normal') }}" required min="0">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="diskon" id="diskon" class="w-full px-4 py-3 border-2 border-border rounded-xl font-sans text-base focus:outline-none focus:border-secondary" value="{{ old('diskon', 0) }}" min="0">
                </div>
            </div>
            <button type="submit" class="w-full px-6 py-3 bg-gradient-to-br from-secondary to-secondary-dark text-white rounded-xl font-semibold border-none cursor-pointer mt-2 hover:opacity-90 flex items-center justify-center gap-1">
                <span class="material-symbols-outlined text-xl">save</span> Simpan Menu
            </button>
        </form>
    </div>
</div>
